...
print(cur_dir)
keyword = 'PDE5a'
storage_dir = '_'.join([keyword,'predict'])
filter_search = '[Title] and "mrna" and "mRNA" not "BAC" not "whole genome"  not "partial" not "synthetic" not "TSA" not "STS" not "patent" not "complete sequence"'
...
#handle = entrz.esearch(db='pub')
print(cur_dir)
storage_dir = '_'.join([keyword,'nonpredict'])
filter_search = '[Title] and "mrna" and "mRNA" not "PREDICTED" not "BAC" not "whole genome"  not "partial" not "synthetic" not "TSA" not "STS" not "patent" not "complete sequence"'
handle = entrez.esearch(db="nucleotide",term=''.join([keyword,filter_search]) ,RetMax = '1000')
match_list = entrez.read(handle) 
print(match_list)
